...
struct graph_parser {
  ...
  uint32_t make_bss_info_index(const Tags& tags, const std::string& uri, uint64_t osm_node_id) {
    valhalla::BikeShareStationInfo bss_info;
    bss_info.set_bss_uri(uri);
    bss_info.set_osm_node_id(osm_node_id);
  ...
  void bss_node(const osmium::Node& node) {
    ...
    bss_nodes_->push_back({n, make_bss_info_index(tags, uri, osmid)});
  }
...
void PBFGraphParser::ParseNodes(const boost::property_tree::ptree& pt,
  ...
  if (pt.get<bool>("import_bike_share_stations", false)) {
    ...
    // Petit handler qui redirige node()/way() vers le parser.
    ...
    for (auto& file : input_files) {
      ...
      location_handler.ignore_errors(); // some way nodes may be missing
