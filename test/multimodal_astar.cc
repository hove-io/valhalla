...
TEST(Standalone, UtrechtMultiModalAStar) {
...
TEST(AstarBss, test_With_Way_ref_11046) {
  ...
  std::vector<std::string>
      expected_route{"Place de la République",  "Boulevard Voltaire",   "Rue Amelot",
                     "Avenue de la République", "Rue des Trois Bornes", "Avenue Parmentier",
                     "Avenue Parmentier",       "Avenue Parmentier",    "Rue Abel-Rabaud"};

  const std::map<size_t, BssManeuverType>&
      expected_bss_maneuver{{3, DirectionsLeg_Maneuver_BssManeuverType_kRentBikeAtBikeShare},
                            {9, DirectionsLeg_Maneuver_BssManeuverType_kReturnBikeAtBikeShare}};
  const std::map<size_t, std::string>& expected_bss_ref{{3, "11046"}, {9, "11035"}};
  const std::map<size_t, std::string>& expected_bss_uri_ref{{3, "way:3127156"},
                                                            {9, "node:6380249071"}};
